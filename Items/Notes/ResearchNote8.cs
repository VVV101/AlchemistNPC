...
She has told me that I must test her '''rooms''. It was rooms with tons of puzzles. She also gave me a tool called ''Portal Gun''.
But the most interesting thing was these cute turrets. I must make one for my own!
First, we need tech which these turrets use. We can get it from the Portal Gun.
Also we need to give them love, so I think we need some sort of ''Companion Cube''.
And as the last part, we need to give them a body. I think  that fire sentry from the Elders army could work.");
...
Для начала, необходима электроника. Мы можем добыть её из Портальной пушки.
Также мы должны подарить им любовь, что означает, что нам необходим ''Куб-компаньон''.
А в качестве последней части, нам необходимо дать ей тело. Думаю, что Огненная турель Древних нам подойдёт.");
			DisplayName.AddTranslation(GameCulture.Chinese, "研究笔记 #8");
			Tooltip.AddTranslation(GameCulture.Chinese, @"曾经, 我在一个有趣的世界. 那是个巨大的实验室. 这个实验室的核心是机器人, 她的名字是“GlaDos”.
她告诉我我必须通过她的“房间”测试. 是个有很多拼图谜题的房间. 她也给我了一个名为“传送枪”的工具.
但最有趣的是这些小可爱炮塔. 我必须自己做一个!
首先, 我们需要知道这些炮塔所用的技术. 我们可以从传送枪上学习.
然后我们也需要爱护他们, 所以我想我们需要一些“伙伴方块”
最后, 我们需要给他们一个身体. 我想那些旧日军团的火焰哨兵也许有用.
