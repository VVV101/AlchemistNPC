...
namespace AlchemistNPC.Items.Weapons
{
	public class CurseofArachna : ModItem
	{
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation(GameCulture.Chinese, "蛛王的咒缚");
		...
		public override void OnHitNPC(Player player, NPC target, int damage, float knockback, bool crit)
		{
			if (target.HasBuff(mod.BuffType("Infested")))
			{
				...
				vel2 *= 3f;
				int p1 = Projectile.NewProjectile(target.position.X-30, target.position.Y-30, vel2.X, vel2.Y, ProjectileID.BabySpider, damage/2, 0, Main.myPlayer);
				Main.projectile[p1].usesLocalNPCImmunity = true;
				Main.projectile[p1].localNPCHitCooldown = 1;
				Vector2 vel4 = new Vector2(1, -1);
				vel4 *= 3f;
				int p2 = Projectile.NewProjectile(target.position.X+30, target.position.Y-30, vel4.X, vel4.Y, ProjectileID.BabySpider, damage/2, 0, Main.myPlayer);
				Main.projectile[p2].usesLocalNPCImmunity = true;
				Main.projectile[p2].localNPCHitCooldown = 1;
				Vector2 vel6 = new Vector2(0, -1);
				vel6 *= 3f;
				int p3 = Projectile.NewProjectile(target.position.X-15, target.position.Y-30, vel6.X, vel6.Y, ProjectileID.BabySpider, damage/2, 0, Main.myPlayer);
				Main.projectile[p3].usesLocalNPCImmunity = true;
				Main.projectile[p3].localNPCHitCooldown = 1;
				Vector2 vel61 = new Vector2(0, -1);
				vel61 *= 3f;
				int p31 = Projectile.NewProjectile(target.position.X+15, target.position.Y-30, vel61.X, vel61.Y, ProjectileID.BabySpider, damage/2, 0, Main.myPlayer);
				Main.projectile[p31].usesLocalNPCImmunity = true;
				Main.projectile[p31].localNPCHitCooldown = 1;
				Vector2 vel7 = new Vector2(-1, 0);
				vel7 *= 3f;
				int p4 = Projectile.NewProjectile(target.position.X-30, target.position.Y, vel7.X, vel7.Y, ProjectileID.BabySpider, damage/2, 0, Main.myPlayer);
				Main.projectile[p4].usesLocalNPCImmunity = true;
				Main.projectile[p4].localNPCHitCooldown = 1;
				Vector2 vel8 = new Vector2(1, 0);
				vel8 *= 3f;
				int p5 = Projectile.NewProjectile(target.position.X+30, target.position.Y, vel8.X, vel8.Y, ProjectileID.BabySpider ,damage/2, 0, Main.myPlayer);
